...
db.publicaciones_feed.insertMany([
  { _id: postIds[0], autor_id: drivers.luis, referencia_despacho: despachoIds[0], tipo_publicacion: "incidencia_multimedia", contenido: "Trafico detenido en Tomas Valle. Estoy desviando por via alterna indicada por operaciones.", multimedia: [{ tipo: "imagen", url: "https://translogix.local/media/trafico_tomas_valle.jpg", size_mb: 2.4 }], reacciones: [{ emoji: "\u{1F62E}", cantidad: 3, usuarios: [] }, { emoji: "\u2705", cantidad: 5, usuarios: [] }], estado: "publicado", createdAt: new Date("2026-06-12T08:42:00Z") },
  { _id: postIds[1], autor_id: drivers.maria, referencia_despacho: despachoIds[1], tipo_publicacion: "reprogramacion", contenido: "Cliente no disponible en recepcion. Operaciones ya coordina nueva ventana de entrega.", multimedia: [], reacciones: [{ emoji: "\u{1F44D}", cantidad: 4, usuarios: [] }], estado: "publicado", createdAt: new Date("2026-06-12T09:15:00Z") },
  { _id: postIds[2], autor_id: users.ana, referencia_despacho: despachoIds[4], tipo_publicacion: "operacion", contenido: "Despacho refrigerado en ruta. Monitorear temperatura cada 20 minutos.", multimedia: [], reacciones: [{ emoji: "\u{1F69A}", cantidad: 2, usuarios: [] }], estado: "publicado", createdAt: new Date("2026-06-13T07:00:00Z") },
  { _id: postIds[3], autor_id: drivers.diego, referencia_despacho: despachoIds[2], tipo_publicacion: "documentacion", contenido: "Guia fisica pendiente de firma. Cliente solicita validacion por correo.", multimedia: [{ tipo: "imagen", url: "https://translogix.local/media/guia_pendiente.jpg", size_mb: 1.1 }], reacciones: [{ emoji: "\u2764\uFE0F", cantidad: 2, usuarios: [] }], estado: "publicado", createdAt: new Date("2026-06-13T10:22:00Z") },
  { _id: postIds[4], autor_id: users.valeria, referencia_despacho: null, tipo_publicacion: "comunicado", contenido: "Recordatorio: todos los conductores deben confirmar inicio y cierre de ruta desde la app.", multimedia: [], reacciones: [{ emoji: "\u2705", cantidad: 8, usuarios: [] }], estado: "publicado", createdAt: new Date("2026-06-16T08:00:00Z") },
]);
...
].map((c, i) => ({ publicacion_id: c[0], autor_id: c[1], texto: c[2], reacciones: [{ emoji: i % 2 === 0 ? "\u{1F44D}" : "\u2705", cantidad: 1 + (i % 3), usuarios: [] }], createdAt: new Date(`2026-06-16T08:${String(10 + i).padStart(2, "0")}:00Z`) })));
